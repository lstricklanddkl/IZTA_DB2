    ...
    Array.from(files).forEach(file => {
        ...
        // TO BE DONE: Validate csv format is good for this application
          ...
          for(var i=0; i<dlength; i++){
            ...
            if (rowRegex.test(data[i])){ //header row will be longer
              // need to convert data types and object
              row = data[i].split(/\s+/g); //split on white space
              // Validate Date 
              ...
              if(!headerRegex.test(data[i])){ console.log("Bad row format: "+ data[i])}
            }
